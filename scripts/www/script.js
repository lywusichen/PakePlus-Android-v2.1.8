...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initChart() {
        ...
        // Prepare chart data
        const sortedData = [...bpData].sort((a, b) => new Date(a.time) - new Date(b.time));
        const labels = sortedData.map(item => formatTimeForChart(item.time));
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            ...
                            title: function(context) {
                                ...
                                return formatTimeForTooltip(sortedData[index].time);
                            }
                        }
                    }
                },
    ...
    function renderRecentReadings() {
        if (bpData.length === 0) {
            recentReadingsContainer.innerHTML = '<p class="empty-message">暂无数据。添加您的第一个读数！</p>';
            return;
        }
        
        // Show only the 10 most recent readings (newest first)
        const recentData = bpData.slice(-10).reverse();
        ...
        recentData.forEach((reading, index) => {
            ...
            // Calculate actual index in bpData (oldest first)
            const actualIndex = bpData.length - 1 - index;
